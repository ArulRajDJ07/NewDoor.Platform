...
public class ApiClientService
{
    ...
    public async Task<List<RuleConfigurationResponse>> GetAllRuleConfigurationsAsync()
    {
        try
        {
            var response = await _httpClient.GetFromJsonAsync<List<RuleConfigurationResponse>>("/api/ruleconfiguration/getall");
            return response ?? new List<RuleConfigurationResponse>();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to load rule configurations");
